...
#include <mach/gpio.h> //Div2D5-OwenHuang-eCompass-AKM8975C_GPIO_ALT-00+
...
static int psensor_value=0;
#endif
//DIV5-BSP-CH-SF6-SENSOR-PORTING04++]

//Div2D5-OwenHuang-SF6_AKM8975C-Framework_Porting-01+{
/*******************************************************************************************************
If you want to enable light/proximity sensor module, please just define the "ENALBE_ALS_PS".
When the module have changed, you just need to modify akm8975.c(kernel) for light/proximity sensor data. 
																			OwenHuang@fihspec.com
********************************************************************************************************/
#define ENABLE_ALS_PS
#ifdef ENABLE_ALS_PS
static int als_enabled = 0; //initial stage(als/ps disabled)
module_param_named(
    als_enabled, als_enabled, int, S_IRUGO | S_IWUSR | S_IWGRP
);

static int ps_enabled = 0; //initial stage(als/ps disabled)
module_param_named(
    ps_enabled, ps_enabled, int, S_IRUGO | S_IWUSR | S_IWGRP
);

extern int read_light_sensor(void); //for read light sensor
extern int read_proximity_sensor(void); //for read proximity sensor
#endif
//Div2D5-OwenHuang-SF6_AKM8975C-Framework_Porting-01+}
...
//Div2D5-OwenHuang-SF6_AKM8975C-Framework_Porting-01+{
	...
	if (als_enabled)
	{
		AKMDBG(KERN_INFO "active light sensor()\n");
		//printk(KERN_INFO "[Owen]active light sensor()\n");
		// SW251-RL-PORTING_SF8_FLAG-00--{
    #ifndef CONFIG_FIH_PROJECT_SF8		
		input_report_abs(data->input_dev, ABS_HAT1X, read_light_sensor());
		#endif
		// SW251-RL-PORTING_SF8_FLAG-00--}			
	}
//DIV5-BSP-CH-SF6-SENSOR-PORTING04++[
	if (ps_enabled)
	{
		AKMDBG(KERN_INFO "active proximity sensor()\n");
		//printk(KERN_INFO "[Owen]active proximity sensor()\n");
		// SW251-RL-PORTING_SF8_FLAG-00--{    
    #ifndef CONFIG_FIH_PROJECT_SF8		
		/*input_report_abs(data->input_dev, ABS_HAT1Y, !read_proximity_sensor()); //temp for FB0 solution(PR2 must modify)*/
		#ifdef CONFIG_FIH_PROJECT_SF4Y6
		//psensor_value=read_proximity_sensor();
		if(read_proximity_sensor() == 0)
		{
			AKMDBG(KERN_INFO "[Colin]proximity sensor 0->9\n");
			psensor_value=9;
		}
		else if (read_proximity_sensor() == 1)
		{
			AKMDBG(KERN_INFO "[Colin]proximity sensor 1->0\n");
			psensor_value=0;
		}
		else
			psensor_value=-1;
		input_report_abs(data->input_dev, ABS_HAT1Y, psensor_value);
		#else
		input_report_abs(data->input_dev, ABS_HAT1Y, !read_proximity_sensor()); //temp for FB0 solution(PR2 must modify)
		#endif	
		#endif
...
static void akm8975_work_func(struct work_struct *work)
{
	...
	//enable_irq(this_client->irq);//Div2D5-OwenHuang-SF6_AKM8975C-Framework_Porting-00*
...
static irqreturn_t akm8975_interrupt(int irq, void *dev_id)
{
	...
	//disable_irq(this_client->irq); //Div2D5-OwenHuang-SF6_AKM8975C-Framework_Porting-00*
...
static void akm8975_early_suspend(struct early_suspend *handler)
{
	...
	atomic_set(&open_flag, 0);
	wake_up(&open_wq);
	//disable_irq(this_client->irq); //Div2D5-OwenHuang-SF6_AKM8975C-Framework_Porting-01*
...
static void akm8975_early_resume(struct early_suspend *handler)
{
	...
	//enable_irq(this_client->irq); //Div2D5-OwenHuang-SF6_AKM8975C-Framework_Porting-01*
	atomic_set(&suspend_flag, 0);
	atomic_set(&open_flag, atomic_read(&reserve_open_flag));
	wake_up(&open_wq);
...
int akm8975_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//Request GPIO for DRDY Interrupt Pin -> configure GPIO
	err = gpio_request(pdata->gpio_DRDY, "AKM8975C_DRDY_pin");
	if (err)
	{
	...
	//Div2D5-OwenHuang-eCompass-AKM8975C_GPIO_ALT-00+{
	err = gpio_tlmm_config(GPIO_CFG(pdata->gpio_DRDY, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
		GPIO_CFG_ENABLE);
	if (err)
	{
	...
	AKMDBG("AKM8975C DRDY Pin GPIO_%d!", pdata->gpio_DRDY);
	
	/* IRQ */
	err = request_irq(client->irq, akm8975_interrupt, /*IRQF_TRIGGER_HIGH*/ IRQF_TRIGGER_RISING, //Div2D5-OwenHuang-SF6_AKM8975C-Framework_Porting-00*
					  "akm8975_DRDY", akm);
	if (err < 0) {
...
//Div2D5-OwenHuang-SF6_AKM8975C-Framework_Porting-01+{
#ifdef ENABLE_ALS_PS
	input_set_abs_params(akm->input_dev, ABS_HAT1X, 0, 3000, 0, 0); //for light sensor; range 0~3000 lux
	input_set_abs_params(akm->input_dev, ABS_HAT1Y, 0, 10, 0, 0); //for proximity sensor; range 0~10 cm
#endif
//Div2D5-OwenHuang-SF6_AKM8975C-Framework_Porting-01+}
	/* Set name */
...
MODULE_AUTHOR("AKM");
MODULE_DESCRIPTION("AKM8975C compass driver");
MODULE_LICENSE("GPL");
