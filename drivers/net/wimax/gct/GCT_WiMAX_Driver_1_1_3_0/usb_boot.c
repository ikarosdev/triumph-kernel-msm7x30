...
#define KERN_PATH		"/etc/gct/zImage"
#define FS_PATH			"/etc/gct/ramdisk.jffs2"
...
static int download_image(struct usb_device *usbdev, struct file *filp, loff_t *pos, u32 img_len, u32 magic_num)
{
	struct dn_header h;
	int ret = 0;
	u32 size;
	...
	if (readn < img_len) {
		printk(KERN_ERR "gdmwm: Cannot read to the requested size. Read = %d Requested = %d\n", readn, img_len);
		ret = -EIO;
	}
out:
