...
#error "Linux kernel version should be greater than 2.6.22 \
...
static spinlock_t k_lock = SPIN_LOCK_UNLOCKED;
static int k_mode_stop = 0;
...
static struct usb_tx *alloc_tx_struct(struct tx_cxt *tx)
{
	...
	t = (struct usb_tx *) kmalloc(sizeof(*t), GFP_ATOMIC);
	if (t == NULL)
		goto out;
...
static struct usb_rx *alloc_rx_struct(struct rx_cxt *rx)
{
	...
	r = (struct usb_rx *) kmalloc(sizeof(*r), GFP_ATOMIC);
	if (r == NULL)
		goto out;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0)
	r->urb = usb_alloc_urb(0);
#else
	r->urb = usb_alloc_urb(0, GFP_ATOMIC);
#endif
	r->buf = (u8 *) kmalloc(RX_BUF_SIZE, GFP_ATOMIC);
...
static struct usb_tx *get_tx_struct(struct tx_cxt *tx, int *no_spc)
{
	...
	*no_spc = list_empty(&tx->free_list) ? 1 : 0;
	...
	#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0)
	t->urb->transfer_flags = USB_QUEUE_BULK;
	#endif
...
static struct usb_rx *get_rx_struct(struct rx_cxt *rx)
{
	...
	#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0)
	r->urb->transfer_flags = USB_QUEUE_BULK;
	#endif
...
static void gdm_usb_send_complete(struct urb *urb, struct pt_regs *regs)
#else
			...
			void (*cb)(void *data), void *cb_data)
{
	...
	if (usbdev->state & USB_STATE_SUSPENDED) {
		...
		queue_work(udev->pm_wq, &udev->pm_ws);
		goto out;
	}
#endif // CONFIG_GDM_USB_PM
...
send_fail:
	...
	gdm_usb_send_complete(t->urb, NULL);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)) \
	&& (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,19))
static void gdm_usb_rcv_complete(struct urb *urb, struct pt_regs *regs)
#else
	...
	if (!urb->status) {
		...
		if (cmd_evt == WIMAX_SDU_TX_FLOW) {
			if (r->buf[4] == 0) {
				...
				list_for_each_entry(t, &tx->sdu_list, list) {
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0)
					usb_submit_urb(t->urb);        
#else
					usb_submit_urb(t->urb, GFP_KERNEL);   
#endif
				}
				/*
...
static int gdm_usb_receive(void *priv_dev, 
			void (*cb)(void *cb_data, void *data, int len), void *cb_data)
{
	struct usbwm_dev *udev = priv_dev;
	struct usb_device *usbdev = udev->usbdev;
	struct rx_cxt *rx = &udev->rx;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0)
	return usb_submit_urb(r->urb);        
#else
	return usb_submit_urb(r->urb, GFP_KERNEL);   
#endif
}
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0)
static void *gdm_usb_probe(struct usb_device *dev, unsigned int ifnum,
...
#endif
{
	...
	bConfigurationValue = usbdev->actconfig->bConfigurationValue;
#else
	...
	printk("Found GDM USB VID = 0x%04x PID = 0x%04x...\n", idVendor, idProduct);
	...
	usbdev->autosuspend_delay = 10 * HZ;
	usbdev->autosuspend_disabled = 0;

	udev->pm_wq = create_workqueue("usb_wimax_pm_wq");
	if (udev->pm_wq == NULL) {
		ret = -1;
		goto out;
	}
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0)
static void gdm_usb_disconnect(struct usb_device *usbdev, void *ptr) 
#else
...
static int gdm_resume(struct usb_interface *intf)
{
	...
	list_for_each_entry(r, &rx->used_list, list)
		usb_submit_urb(r->urb, GFP_KERNEL);
